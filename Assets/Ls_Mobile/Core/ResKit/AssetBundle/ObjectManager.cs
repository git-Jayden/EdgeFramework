...
namespace EdgeFramework
{
    public class ObjectManager : Singleton<ObjectManager>
    {
        ...
        protected ResouceObj GetObjectFromPool(uint crc)
        {
            ...
            {
                ResouceManager.Instance.IncreaseResouceRef(crc);
        ...
        {
            ResouceObj resObj = null;
            if (asyncResObjs.TryGetValue(guid, out resObj) && ResouceManager.Instance.CancleLoad(resObj))
            {
        ...
        public GameObject InstantiateObject(string path, bool setSceneObj = false, bool bClear = true)
        {
            ...
            if (resouceObj == null)
            {
                ...
                resouceObj = ResouceManager.Instance.LoadResouce(path, resouceObj);
            ...
            object param1 = null, object param2 = null, object param3 = null, bool bClear = true)
        {
            ...
            long guid = ResouceManager.Instance.CreatGuid();
            ...
            ResouceManager.Instance.AsyncLoadResouce(path, resObj, OnLoadResouceObjFinish, priority);
            return guid;
        }
        /// <summary>
        /// 资源加载完成回调
        ...
        public void ReleaseObject(GameObject obj, int maxCacheCount = -1, bool destroyCache = false, bool recycleParent = true)
        {
            ...
            {
                resouceObjDic.Remove(tempID);
                ResouceManager.Instance.ReleaseResouce(resObj, destroyCache);
                resObj.Reset();
                resouceObjPool.Recycle(resObj);
            }
            else //回收到对象池
            {
                ...
                if (maxCacheCount < 0 || st.Count < maxCacheCount)
                {
                    st.Add(resObj);
                    resObj.already = true;
                    ResouceManager.Instance.DecreaseResouceRef(resObj);
